...
public class HotelServiceImpl implements HotelService {
	...
	public List<Chambre> getReservationFromLessCrowdedHotel(LocalDate date) {
		...
		List<Reservation> reservations = hotelRepo.getReservationsFromDate(date);
		...
		for(Hotel h: hotelRepo.findAllById(hotelByOccupation.keySet()) ) {
			if(chambres.size() >= 3) break;
			
			chambres.addAll(hotelRepo.getChambresWithoutReservationInDate(date));
		}
	...
	public Map<Integer, Integer> getOccupationByHotel(int indexMonth) {
		...
		for (Reservation r : reservations) {
			...
			int nbDays = (int) (getDaysInMonthOfDateRange(indexMonth, r.getStartDate(), r.getEndDate()) / totalChambre) * 100;
